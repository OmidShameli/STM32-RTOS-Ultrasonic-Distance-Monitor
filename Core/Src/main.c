...
/* Definitions for xDistanceQueue */
osMessageQueueId_t xDistanceQueueHandle;
const osMessageQueueAttr_t xDistanceQueue_attributes = {
  .name = "xDistanceQueue"
};
/* USER CODE BEGIN PV */
...
void Start_AlertTask(void *argument);
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
	...
	{
		if(echo_done == 0 && HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_7) == GPIO_PIN_SET)
		{
...
int main(void)
{
  ...
  /* Create the queue(s) */
  /* creation of xDistanceQueue */
  xDistanceQueueHandle = osMessageQueueNew (1, sizeof(uint32_t), &xDistanceQueue_attributes);
  ...
  AlertTaskHandle = osThreadNew(Start_AlertTask, NULL, &AlertTask_attributes);
...
void Start_SensorTask(void *argument)
{
  ...
  for(;;)
  {
	  ...
	  if(echo_done == 2)
	  {
		  uint32_t elapsed_US = echo_end - echo_start ;
		  distance = (elapsed_US *34) / 2000;
	  }


	  xQueueOverwrite(xDistanceQueueHandle, &distance);
	  vTaskDelay(pdMS_TO_TICKS(100));
  }
  /* USER CODE END 5 */
}

/* USER CODE BEGIN Header_Start_AlertTask */
/**
* @brief Function implementing the AlertTask thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_Start_AlertTask */
void Start_AlertTask(void *argument)
{
  /* USER CODE BEGIN Start_AlertTask */
  /* Infinite loop */

	uint32_t distance = 0;
  for(;;)
  {
	  xQueuePeek(xDistanceQueueHandle, &distance, portMAX_DELAY);
